...
def fetch_product_htmls(brand, max_products=200):
    ...
    app_logger.debug(
        f"crawl product htmls of brand {brand}, {max_products} products each finished")
    app_logger.debug(f"{len(product_ids)} products are crawled")

    # crawl each product html by id
    htmls = {}
    with concurrent.futures.ThreadPoolExecutor(max_workers=90) as executor:
        bar = tqdm(executor.map(
            crawl_musinsa_product_html, product_ids), total=len(product_ids))
        bar.set_description(f"crawling {brand} products:")
        for pid, html in zip(product_ids, bar):
            htmls[pid] = html
...
def crawl(brands):
    brand_groups = np.split(np.array(brands), len(brands) / 20)
